...
open class ApacheHttpRequestInfo(protected val req: HttpRequest, protected val context: HttpContext) : HttpRequestInfo {
    ...
    // When HttpRequest.requestLine.url is used, remove querystring since the return value of the method may be used by span name
    private fun getPath(): String? = if (req is HttpUriRequest) req.uri.path else req.requestLine.uri.split("?")[0]
}